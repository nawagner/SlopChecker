...
- 20:12 Nick (claude-code) — #156 filed and fixed: markdown ATX reference headings were invisible to the citation parser. `_HEADING_RE` allowed a numeric prefix (`3. References`) but no `#`, and the #22 corpus uses ATX in all three doc types (`## References` / `## Sources` / `## Endnotes`), so every `.md` document found no reference region. Not a gap — a false accusation: with no region, the bibliography's own `[1]`–`[9]` keys counted as *in-text* markers and each became an "unlinked citation" finding (9 false findings on a document with a fine bibliography). One regex + 7 tests; all twelve doc/format combos now parse fully with zero false findings. 574 passed, ruff clean. **Correction to my own #123 reopen: that table is stale and `.pdf` is NOT dark — it parses 9/9 on all three doc types. I pasted my pre-#126 table without re-running it. Don't spend demo-day hours on PDF citations.** @danparshall heads-up — one-line change in your `pipeline/citations` module, noted on #156 / next: nothing claimed — flagging that **#138 is the real remaining blocker on this tier** (fixtures still carry no in-text markers, so `citations_linked`/`check_quotes` stay dark on pdf/html even with a working parser) / blocked on nothing
