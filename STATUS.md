...
- 17:10 Emerson (claude-code) — unsourced-quant-claims judgment moved into the LLM (#13): the mechanical quantitative&&uncited flag misfired on every budget line ('$429,401 over 3 years' needs no footnote); the claims lens (v0.2) now emits needs_source per claim (rule 6: true only for assertions about the world a reviewer would expect a citation for; applicant's own budget/deliverables/timeline are theirs to make), mapping flags quantitative && needs_source && uncited, missing field defaults to quiet. Also hardened against models omitting claim ids (fallback CL<n> instead of a KeyError). Verified live on proposal_climate.pdf: 18 claims, exactly 2 flagged, both genuinely empirical. Taxonomy widened again 15 -> 25 (community_development, economic_policy, agriculture_food, transportation_infrastructure, national_security, civil_rights, water_oceans, mental_health, child_family, arts_culture); Aldergrove RFP now classifies climate_environment 0.95 + community_development/water_oceans in the deterministic lane, proposal_climate climate_environment 0.90 (both were headed for 'other') / next: merge the stacked branch / blocked on CI trigger flakiness

- 16:35 Dominique (claude-code) — web rubric upload, whole slice (#148): `POST /check` takes an optional `rubric` multipart field, ingested through the normal `ingest()` path and set on the context exactly as `cli.py` does (`ctx.rubric`), with `report.solicitation` stamped from the rubric's filename — CLI parity, including leaving `ctx.solicitation` alone when no explicit label was given. Landing page gets an "Attach your solicitation (optional)" row inside the existing upload form (one upload, not a second step), sent as the `rubric` field; drag-and-drop still targets the proposal. Upload errors on the second field are prefixed `rubric:` by the API and the page turns that into "Couldn't read your solicitation: …" so a bad RFP never reads as a bad proposal — the proposal's own error copy is unchanged. **Copy change in someone else's module, flagged here and on #148/#149:** `checks_rubric.py`'s gap reason said `no rubric supplied (--rubric)` — a CLI flag in a row funders now read on the website — so it's `no solicitation or rubric supplied — compliance not checked` on both surfaces (@etbrooking, revert freely if you disagree; three tests updated). Also fixed a nit from #149 the web verification surfaced: the caption/source-filename space existed only in the flex gap, so copy-paste and the PDF text layer got "requiresaldergrove-rfp.md" — now in the markup. Verified end to end in a real browser against a local uvicorn + static-file server, not just TestClient: both files posted from the actual form JS → report with the two-quote pair and `Checked against:` header; proposal-only → skipped compliance row and no solicitation stamp; `.exe` rubric → 422 surfaced as the new sentence. Dark mode and 375px checked, no horizontal overflow. 7 new web tests, 581 passed / next: the #148 acceptance box I can't tick from here — "verified once on slop-checker.com" needs the Railway + Worker deploy, so someone with deploy eyes should re-run the two-file upload after this lands / blocked on nothing (the Worker needs no change: `/api/*` is proxied as a raw request, multipart included).
...
- 20:40 Dan (fable) — #142 smoke script pays for itself an hour after merging:
  ...
  on nothing.
- 16:50 Emerson (claude-code) — two fixes from live review: (1) taxonomy widened 5 -> 15 topics (labor_economics, education, public_health, climate_environment, democracy_governance, media_information, global_development, housing_urban, criminal_justice, immigration) in DEFAULT_TAXONOMY + taxonomy.example.toml — the flagship labor-economics fixture was classifying as 'other' because labor wasn't in the closed set the LLM classifier is allowed to use; both tiers share the taxonomy so the deterministic matcher gains the same vocabulary (Nick's module, noted on #15). (2) Pangram marks stop wallpapering short docs: each surfaced window's anchor now trims to its opening sentences (<=300 chars, sentence-boundary cut), note states 'opening of a N-word passage', full span stays in evidence — five whole windows were covering essentially all of a 2-page doc / next: nothing queued / blocked on nothing
...
- 16:20 Emerson (claude-code) — report.json section of the evidence report is now collapsed by default (native <details>, summary styled as the section heading) with a Download button that emits byte-for-byte the JSON the page was rendered from, named <stem>.report.json to match the CLI. Collapsed state also keeps the printed PDF lean. (#19) / next: nothing queued / blocked on nothing
...
- 16:05 Emerson (claude-code) — #20 batch summary view built on the existing CLI batch mode: new report/batch.py (summarize_for_batch row extractor + render_batch — self-contained sortable/filterable HTML with client-side CSV/JSON export of exactly the visible rows, file links into each per-doc evidence report, reuses report.css); batch runs now write summary.html + summary.json beside summary.csv (CSV gains doc_type/pangram/citation_flags/similarity columns after the existing ones). Ingest gaps render as 'not read' rows with the reason as tooltip. Smoke-tested on 4 fixture PDFs end to end; 5 new tests, suite green / next: PR, then demo-doc dry run / blocked on nothing
...
- 16:09 Dominique (claude-code) — rubric evidence renders (#149): #145's `rubric_quote` was sitting invisible in the evidence dict; the card now shows the two-quote pair — funder's requirement (accent rule, captioned with the rubric filename it came from) above the proposal's violating line (lane-coloured rule), both verbatim, stacked because the rail is 22rem and two narrow columns re-wrap both quotes. The proposal side reuses `anchor.quote` even though it's already marked in the body, because in print the rail lands *after* the document and the card has to carry the whole comparison. **The pair deliberately survives rail condensing** (the kv table doesn't — that's recoverable from the one-liner; a clamped quote misrepresents evidence, and hidden-behind-a-click means the strongest thing on the page is invisible on arrival and in screenshots). `evidence` renders through an opt-in table (`_EV_QUOTES` key→caption+source-key, `_EV_FIELDS` key→label+formatter), never a dict dump — parse internals stay out of the card and stay visible in the embedded report.json; a new check with evidence worth showing adds a row. `ceiling_usd`/`budget_total_usd` render as `$75,000`/`$90,000` in a colourless `.v-fact` class: an input to the check tinted red reads as a second failing check. Header says `Checked against: <solicitation>`, silent when unset so the skipped-rubric ledger row stays the one place the negative case is stated. Verified in light + forced-dark and in the real print PDF (pair intact on one page, `break-inside: avoid`; the rules are `border-left`, not backgrounds, so they survive a browser print with backgrounds off). `demo-report.html` regenerated via the renderer / **note: the demo asset renders from `tests/fixtures/sample_report.json`, which has no rubric finding — so the landing-page specimen gets the CSS and the header line but not the two-quote moment. Making it show means either adding a row to that shared fixture (four `worker/test/*.ts` + `test_models` import it, `docs/DATA_MODEL.md` calls it canonical) or repointing the specimen at the climate run; asked on #149 rather than deciding for `worker/test`'s owners** / next: whichever of those two the answer on #149 is / blocked on nothing. Local aside: 7 `test_upload_transcript.py` failures here are this laptop's git being too old for `git init -b` — identical on clean `origin/main`, not from this change; 549 passed.
...
- 19:58 Dan (air) — #17 budget-feasibility Phase 3 landed on `danparshall/17-budget-lens` (test-first per the plan): new `pipeline/budget_feasibility/{llm,prompts,check}.py` mirroring `claim_support/` — `Transport` protocol + typed transport errors + `AnthropicTransport` with lazy `anthropic` import; `LENS_OUTPUT_SCHEMA` (strict `additionalProperties:false` throughout, nullable-typed everywhere the lens is allowed to emit null) + `build_budget_feasibility_prompt(lens, doc_text)` (ships the lens's own one-shot from its `### Input`/`### Output`); `BudgetFeasibilityCheck.run` = load_lens → LLM call → parse → quotecheck-drop → `evaluate_lens_output`. `_call_with_retry` inline per #37 (429/5xx retry, auth/client/refusal surface immediately). Malformed payload or out-of-enum role → errored ledger row, never a stack trace (CLAUDE.md degrade-to-gaps); ledger detail surfaces quote-drops rather than a second row shape (matches `claim_support`'s aggregated-detail pattern). `@register(id="budget_feasibility", tier="llm", est_cost_usd=0.03)` — added to `registry.CHECK_PACKAGES`. 10 new mocked-transport tests appended to `tests/test_budget_feasibility.py` (`FakeTransport` scripting judge-alignment + exception replay; Meridian happy-path loaded from the lens's `example_output` so any lens-md drift trips `test_lenses.py` first not here; hallucinated-quote drop, transient-retry-then-succeed + retry-exhaustion, permanent auth error, malformed enum, missing key, discovery). 34 file / 417 suite green; ruff+format+mypy clean on new code. Phases 4 (fixtures + smoke) + 5 (ai-log + PR) still open. Coordination with @nawagner on `project.stated_total_usd` name unchanged / next: Phase 4 fixture pair (underfunded + reasonable Meridian variants) → Phase 5 PR / blocked on nothing.

- 19:37 Dan (air) — #17 budget-feasibility Phases 1 + 2 landed on `danparshall/17-budget-lens` (test-first per the plan): `src/slopchecker/lenses/budget_feasibility.md` (commit 655ab67, generic `tests/test_lenses.py` parametrized quote-verbatim gate green) + `src/slopchecker/pipeline/budget_feasibility/{__init__,benchmarks_us,evaluate}.py` + `tests/test_budget_feasibility.py` (commit 7291630, 24 new unit tests: shortfall_factor math on the Meridian planted defect at p20/p80, `personnel_underfunded` flag boundary (fires at 3.0, silent at 2.9), `pairing_ratio_usd_per_unit` single + many-to-many-sum, unpaired_scope/budget derivation, `unfunded_quantitative_commitment` gated on quantity>0, `sum_of_lines_matches_stated_total` ±$1 tolerance + null-not-emitted, end-to-end shape assertion on the full Meridian LensOutput, US_2026 bands locked as a reviewer contract). Corrected plan's expected_p20 ~$163,840 to $163,520 after hand-verification (0.25×95K + 2×52K = 127,750 × 1.28); shortfall factor ≈4.088 unchanged. Frontmatter id normalized to `budget_feasibility` (underscore, matches filename stem) — plan doc's dashed form was a transcription slip. Full suite 407 green; ruff check + format clean; new-code mypy clean (5 pre-existing mypy errors in config.py/runner.py/resolution.py/web.py untouched — non-blocking in CI per ci.yml line 29). Pushed to origin. Coordination note still open with @nawagner: `project.stated_total_usd` overlaps with #109's arithmetic remit — align field name before either PR merges / next: Phase 3 orchestrator (mirror `pipeline/claim_support/{check,llm,prompts}.py`, mocked-transport tests for LLM success + quotecheck-drop + transport-error gap) → Phase 4 fixtures (underfunded + reasonable Meridian variants) → Phase 5 PR / blocked on nothing.

- 19:02 Dan (air) — #17 budget-feasibility lens design conversation on `danparshall/17-budget-lens`, no code yet. Split #17 into two work items: arithmetic half spun out to #109 (assigned Nick, `checks/` module — line-items-sum, indirect rate, totals-match-narrative), feasibility half stays on #17 as an LLM lens (`lenses/`) + Python evaluator (`pipeline/budget_feasibility/`) that applies wide US 20th–80th-percentile salary bands from BLS/Chronicle/NIH. Lens does pure extraction+pairing (no arithmetic, no verdicts, matches `claims.md` philosophy); evaluator does bounded US-only judgment with the assumption printed in every `Finding.evidence`. Locked: JSON schema (personnel_lines with roles_named enum, non_personnel_lines, scope_commitments with quantity+unit, many-to-many pairings, top-level project.stated_total_usd/duration_yrs), Meridian few-shot with a planted underfunded personnel line ($40K for 2 postdoc-yr → shortfall factor ≈ 4.1x against p20), evaluator interface (`evaluate_lens_output()` + `US_2026` benchmarks + `stated < expected_p20 / threshold=3.0` flag rule to keep false-positives honest). Live end-to-end tests deferred to #130 (env-gated, mine to write next). Full design + implementation plan in `docs/active/17-budget-lens/`. Coordination note for @nawagner: `project.stated_total_usd` field overlaps with #109's arithmetic remit — align field name before either PR merges / next: hand off the implementation plan to next agent to draft the lens .md + evaluator scaffolding / blocked on nothing.
