...
- 19:31 Nick (claude-code) — #143 CI wall clock 35s -> ~21s expected. Measured the per-job split first: `test` 35s (setup 8 / pytest 7.8 / integration 11.6), `live` 26s, `worker` 20s — wall = max(), and it flips between `test` and `live` run to run, so shaving one alone often buys nothing. Two changes: (1) `integration` gets its own job — it was 11.6s serialised behind a 7.8s unit run for no reason. Proved it's hermetic first (passes identically under blocked DNS, 9.39s vs 9.27s), which is what makes it safe to split: unlike `live` it SHOULD be required, so the ruleset set becomes `test` AND `integration` — commented on #43, because adding the job without the ruleset entry is the #114 mistake in a new costume. (2) `pytest -m live -n 4`: xdist belongs here and only here — unit suite is CPU-bound (~85%), live is 24 tests at ~10% CPU waiting on four hosts. 4 is measured: serial 17.7 / n2 13.5 / n3 11.7 / n4 10.5 / n6 12.5 / n8 12.4 — it gets *worse* past 4, so that's a ceiling not a starting point. 24/24 green on 3 consecutive -n 4 runs. Also recorded: #132 is NOT a CI lever — test_web is 0.147s on the runner vs 2.35s locally, so the local profile badly overstates it; it stays a dev-loop fix / next: PR / blocked on nothing.
