"""Transport layer for the claim-support check (#11).

Thin wrapper on the Anthropic Messages API. Mirrors the shape of
``detect/pangram.py`` per the #37 design comment:

- Typed transport exceptions (401/402 → auth, 429 → rate-limit, 5xx →
  server, everything else 4xx → client). ``_call_with_retry`` in ``check.py``
  retries 429/5xx with exponential backoff and surfaces auth/client errors
  immediately.
...
- Structured output via ``output_config.format`` (JSON schema). The
  claude-api skill's ADR: assistant-turn prefills return 400 on Opus 4.6+,
  and the SlopChecker rule bans free-text results — so a closed JSON
  schema is the only shape that satisfies both.

The transport itself does not know about judge vs refuter roles; it takes
``system`` + ``user`` + ``schema`` + ``model`` and returns the validated
JSON. The ``role`` argument is a diagnostic label passed through so tests
can assert call-alignment, not a wire concept.
"""
...
class TransportAuthError(TransportError):
    """401 / 402 — invalid key or exhausted credits. Never retried."""
...
class TransportRefusal(TransportError):
    ...
    Distinct from a network refusal (that's an auth or client error). A
    policy refusal on a claim-support call is rare but real; the runner
    turns it into an errored ledger row rather than pretending the check
    ran.
    """
...
class AnthropicTransport:
    """The real transport. Uses the Anthropic SDK's client (auto-retries 429/5xx
    on top of ours — safe, our retry is the visible policy) and
    ``output_config.format`` for structured output.

    Constructed lazily by the check so unit tests never import ``anthropic``.
    """

    def __init__(self, *, api_key: str, max_output_tokens: int = 2048) -> None:
